...
OVERLAPS_PROGRESS_PATH = os.path.expanduser("~/git/MicrobiomeKG/config/s1_raw_graph/overlaps_progress.json")
EVALUATED_PAIRS_PATH = os.path.expanduser("~/git/MicrobiomeKG/config/s1_raw_graph/evaluated_pairs.tsv")
...
def validate_overlaps(state: GraphState):
    ...
    new_all_overlaps = state.get("all_overlaps", []) + state["current_overlaps"]
    
    os.makedirs(os.path.dirname(EVALUATED_PAIRS_PATH), exist_ok=True)
    with open(EVALUATED_PAIRS_PATH, "a", encoding="utf-8") as f:
        f.write(f"{label_a}\t{label_b}\n")
        
    if state["current_overlaps"]:
        with open(OVERLAPS_PROGRESS_PATH, "w", encoding="utf-8") as f:
            json.dump(new_all_overlaps, f)
    
    return {
        "all_overlaps": new_all_overlaps,
        "current_index": idx + 1,
        "errors": "",
        "retries": 0,
        "current_overlaps": []
...
def main():
    ...
    processed_set = set()
    if os.path.exists(EVALUATED_PAIRS_PATH):
        with open(EVALUATED_PAIRS_PATH, "r", encoding="utf-8") as f:
            for line in f:
                parts = line.strip().split('\t')
                if len(parts) == 2:
                    processed_set.add((parts[0], parts[1]))
                    processed_set.add((parts[1], parts[0])) 

    all_overlaps = []
    if os.path.exists(OVERLAPS_PROGRESS_PATH):
        with open(OVERLAPS_PROGRESS_PATH, "r", encoding="utf-8") as f:
            all_overlaps = json.load(f)
            
    remaining_pairs = [p for p in valid_label_pairs if (p[0], p[1]) not in processed_set]
    total_pairs = len(remaining_pairs)

    if total_pairs == 0:
        print("All pairs have been processed. Generating final TSV.")
        os.makedirs(os.path.dirname(OUTPUT_TSV_PATH), exist_ok=True)
        final_tsv = format_final_tsv(all_overlaps)
        with open(OUTPUT_TSV_PATH, "w", encoding="utf-8") as file:
