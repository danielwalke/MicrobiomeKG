...
def main():
    ...
    ## Step 1
    print("\n--- Creating workspace ---")
    subprocess.run(['java -jar BioDWH2-v0.6.8.jar -c ~/git/MicrobiomeKG/1_raw_knowledge_graph/workspace'], capture_output=True, text=True)
    
    print("\n--- Changing config based on user input ---")
    generate_config(selected_sources)
    print("\n--- Updating workspace ---")
    subprocess.run(['java -jar BioDWH2-v0.6.8.jar -u ~/git/MicrobiomeKG/1_raw_knowledge_graph/workspace'], capture_output=True, text=True)
    print("\n--- Creating raw neo4j database workspace ---")
    subprocess.run(['java -jar BioDWH2-Neo4j-Server-v1.3.2.jar --create ~/git/MicrobiomeKG/1_raw_knowledge_graph/workspace'], capture_output=True, text=True)
    print("\n--- Starting raw neo4j database workspace ---")
    subprocess.run(['java -jar BioDWH2-Neo4j-Server-v1.3.2.jar --start ~/git/MicrobiomeKG/1_raw_knowledge_graph/workspace'], capture_output=True, text=True)
    ...
    subprocess.run(["docker compose -f ~/git/MicrobiomeKG/2_raw_metagraph/docker-compose.yml up -d "])
    subprocess.run(["python -m 2_raw_metagraph.extract_metagraph --suri bolt://localhost:8083 --suser neo4j --spass neo4j --turi bolt://localhost:7688 --tuser neo4j --tpass "])
    ...
    subprocess.run(["docker compose -f ~/git/MicrobiomeKG/3_metaconcept_graph/docker-compose.yml up -d "])
    subprocess.run(["python -m 3_metaconcept_graph.extract_concepts --suri bolt://localhost:7688 --suser neo4j --spass neo4j --turi bolt://localhost:7689 --tuser neo4j --tpass "])
    ...
    subprocess.run(["python -m 4_filtered_metaconcept_graph.identify_relevant_properties.py"])
    print("\n--- Filtering Metaconceptgraph based on selected intresting properties ---")
    subprocess.run(["docker compose -f ~/git/MicrobiomeKG/4_filtered_metaconcept_graph/docker-compose.yml up -d"])
    subprocess.run(["python -m 4_filtered_metaconcept_graph.filter_metagraph"])
    ...
    subprocess.run(["docker compose -f ~/git/MicrobiomeKG/5_filtered_knowledge_graph/docker-compose.yml up -d"])
    subprocess.run(["python -m 5_filtered_knowledge_graph.filter_knowledge_graph"])
