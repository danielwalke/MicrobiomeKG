...
1.  **Initialized raw_metagraph directory**: Confirmed location at `/Users/danielwalke/git/microbiomeprocheck/step_2_raw_metagraph`.
...
6.  **Enhanced Extraction**: Updated `extract_metagraph.py` to infer and store **property keys and data types** for all nodes and relationships. Inference is handled by Neo4j itself
 (python -m step_2_raw_metagraph.extract_metagraph.py --suri bolt://localhost:8083 --suser neo4j --spass neo4j --turi bolt://localhost:7688 --tuser neo4j --tpass "")