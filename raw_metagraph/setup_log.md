...
3.  **Extraction Script (In Progress)**: Developing `extract_metagraph.py` to pull schema information from the source BioDWH2 Neo4j and push it to the target container.
...
6.  **Enhanced Extraction**: Updated `extract_metagraph.py` to infer and store **property keys and data types** for all nodes and relationships. Inference is handled entirely within the Python logic.
